...
internal class TemplateStringDbContext
{
    ...
    private List<TemplateString> TemplateStringCollection { get; } = new()
    {
        ...
        new TemplateString(Guid.NewGuid(), "TextIsTooLongException", $"Текст слишком длинный, максимальное количество символов: MaxSymbols, ваш текст: TextLength. {Environment.NewLine}The text is too long, maximum number of characters: MaxSymbols, your text: TextLength.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "UnknownErrorWeWillTryToFixItAsSoonAsPossible", $"Неизвестная ошибка, мы постараемся исправить ее как можно скорее. {Environment.NewLine}Unknown error, we will try to fix it as soon as possible.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "UnknownMessageTypeException", $"Простите, получен неизвестный тип сообщения. {Environment.NewLine}Sorry, unknown message type received.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "VoiceMessageTypeNotImplementedException", $"Простите, голосовые сообщения пока не поддерживаются. {Environment.NewLine}Sorry, voice messages are not supported yet.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "RateLimitException", $"Простите, переводчик сейчас пользуется таким спросом, что нам заблокировали ресурсы. Через минуту все придет в норму, приносим извинения! {Environment.NewLine}Sorry, the translator is in such high demand right now that we've been blocked from resources. In a minute everything will be back to normal, we apologize!", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "ModelOverloadedException", $"Упс.. Несмотря на то что у нас повышенный приоритет на взаимодействие с chatGPT, он все равно не смог выполнить запрос из-за того что перегружен. Приносим свои извинения! Попробуйте через несколько минут, пожалуйста. {Environment.NewLine}Oops.. Despite our high priority on interaction with chatGPT, it still couldn't fulfill the request due to being overloaded. We apologize! Please try again in a few minutes.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
...
🇷🇺 - 🇺🇿 @gpt_translator_rus_uzb_bot", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "ReceivedAdditionalContextNowPleaseChooseLanguage", $"Мы получили предоставленный вами дополнительный контекст, теперь, пожалуйста, выберите один из предоставленных ниже вариантов языка. {Environment.NewLine}We have received the additional context you provided. Now, please select one of the language options provided below.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "ReceivedAdditionalContextNowPleaseChooseStyle", $"Мы получили предоставленный вами дополнительный контекст, теперь, пожалуйста, выберите один из предоставленных ниже вариантов стиля. {Environment.NewLine}We have received the additional context you provided. Now, please select one of the style options provided below.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "SomethingWentWrongWorkingOnIt", $"Похоже, что-то пошло не так, мы постараемся исправить это как можно скорее. {Environment.NewLine}It seems, something went wrong. We're trying to fix it as soon as possible.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
        ...
        new TemplateString(Guid.NewGuid(), "EntityNotFoundInDb", $"Ваш перевод не был найден в базе данных, пожалуйста, начните новый. {Environment.NewLine}We couldn't find your translation in the database, please, start a new one.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),

        new TemplateString(Guid.NewGuid(), "WaitAMomentPlease", $"Подождите, пожалуйста. {Environment.NewLine}Wait a moment, please.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),

        new TemplateString(Guid.NewGuid(), "Please, write the text that needs to be translated.", $"Пожалуйста, напишите текст, который нужно перевести. {Environment.NewLine}Please, write the text that needs to be translated.", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),

        new TemplateString(Guid.NewGuid(), "Hello, new user", $"Здравствуй, новый пользователь. {Environment.NewLine}Hello, new user", "", TemplateLanguage.Rus_Eng, EnvironmentType.Any),
